...
const detectCycle = (g) => {
  const visited = Array(g.vertices.length).fill(false)
  const recorded = Array(g.vertices.length).fill(false)

  const detectCycleRecur = (g, i, vis, rec) => {
    if (vis[i] == false) {
      vis[i] = true
      rec[i] = true
