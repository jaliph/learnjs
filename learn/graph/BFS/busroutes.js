...
const numBusesToDestination = function (routes, source, target) {
  ...
  for (const [bus, route] of routes.entries()) {
    for (stop of route) {
      const busId = `b${bus}`
      addEdge(stop, busId, 1)
      addEdge(busId, stop, 0)
    }
  }
  ...
  while (k < q.length) {
    const [point, distance] = q[k++]

    if (point == target) return distance

    for (const nei of g[point]) {
      const [next, weight] = nei
      if (!visited.has(next)) {
        visited.add(next)
        q.push([next, distance + weight])
      }
    }
  }
