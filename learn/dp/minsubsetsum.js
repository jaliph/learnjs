...
let getMinDiff = function(num) {
  let getMinDiffRecur = (arr, i, sum1, sum2) => {
    if (arr.length == 0) return 0

    if (i == arr.length) return Math.abs(sum1 - sum2)
    
    let diff1 = getMinDiffRecur(arr, i + 1, sum1 + arr[i], sum2)
    let diff2 = getMinDiffRecur(arr, i + 1, sum1, sum2 + arr[i])
...
let getMinDiffDP = function(num) {
  ...
  let dp = Array(num.length).fill(0).map(() => Array(requiredSum + 1).fill(0))
