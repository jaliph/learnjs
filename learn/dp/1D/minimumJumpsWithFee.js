const minFeeBrute = (steps, n) => {
  const minFeeRecur = (steps, n, i) => {
    ...
    const oneJump = minFeeRecur(steps, n, i - 1) + steps[i]
    const twoJump = i - 2 >= 0 ? steps[i - 2] + minFeeRecur(steps, n, i - 2) : steps[i - 2 + n] + minFeeRecur(steps, n, i - 2)
    const threeJump = i - 3 >= 0 ? steps[i - 3] + minFeeRecur(steps, n, i - 3) : steps[i - 3 + n] + minFeeRecur(steps, n, i - 3)
...
const minFee = (costs, n) => {
  ...
  for (let i = 3; i <= n; i++) {
    const oneStep = cost[i - 1] + dp[i - 1]
    const twoStep = cost[i - 2] + dp[i - 2]
    const threeStep = cost[-3] + dp[i - 3]
...
// Driver code
function main () {
  const inputs = [[1, 2, 5, 2, 1, 2], [2, 3, 4, 5],
