...
UndergroundSystem.prototype.checkOut = function (id, stationName, t) {
  const [timeStarted, startStation] = this.transitState.get(id)
  this.transitState.delete(id)
  const [lastStationAverage, count] = this.stationDuration.get(`${startStation}-${stationName}`) || [0, 0]

  // console.log('arre', lastStationAverage, count)
  const timeTaken = t - timeStarted
...
UndergroundSystem.prototype.getAverageTime = function (startStation, endStation) {
  const [totalTime, count] = this.stationDuration.get(`${startStation}-${endStation}`)
