...
const maxPerformance = function (n, speed, efficiency, k) {
  const engineers = speed.map((val, i) => [val, efficiency[i]])

  engineers.sort((a, b) => b[1] - a[1])

  const h = new Heap((a, b) => a[0] - b[0])
  let speedSum = BigInt(0); let result = BigInt(0)
  for (let i = 0; i < n; i++) {
    ...
    if (h.size > k) {
      const minSpeed = h.pop()
      speedSum -= BigInt(minSpeed)
    }

    const currResult = BigInt(speedSum) * BigInt(min)
