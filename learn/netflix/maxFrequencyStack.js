...
FreqStack.prototype.push = function (showName) {
  ...
  } else { this.frequency[showName] = 1 }

  if (this.frequency[showName] > this.maxFrequency) { this.maxFrequency = this.frequency[showName] }

  if (this.group.has(this.frequency[showName])) { this.group.get(this.frequency[showName]).push(showName) } else {
...
FreqStack.prototype.pop = function () {
  if (this.group.has(this.maxFrequency)) {
    const show = this.group.get(this.maxFrequency).pop()
    this.frequency[show]--
